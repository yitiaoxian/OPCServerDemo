    ...
    public class NodeManager : CustomNodeManager2
    {
        ...
        public override void CreateAddressSpace(IDictionary<NodeId,IList<IReference>> externalReferences)
        {
            lock (Lock)
            {
                ...
                FolderState root = CreateFolder(null, "XDataServer", "XDataServer");
        ...
        {
            BaseDataVariableState variableState = CreateVariable(parent, path, name, BuiltInType.Double, ValueRanks.Scalar);
            if (peers != null)
            {
                foreach (NodeState peer in peers)
                {
                    peer.AddReference(ReferenceTypes.HasCause, false, variableState.NodeId);
                    variableState.AddReference(ReferenceTypes.HasCause, true, peer.NodeId);
                    peer.AddReference(ReferenceTypes.HasEffect, true, variableState.NodeId);
                    variableState.AddReference(ReferenceTypes.HasEffect, false, peer.NodeId);
                }
            }
